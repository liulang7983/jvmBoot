...
public class Jmm02_CpuCache {
    ...
    /**
     空间局部性（Spatial Locality）：如如果一个存储器的位置被引用，那么将来他附近的位置也会被引用。
     比如顺序执行的代码、连续创建的两个对象、数组等
     所以此时循环long[i][j]的时候,此时先循环i在再i里面循环j，此时循环j的时候会把他后续的连续空间也取过来存放到L1，速度更快
    ...
    public static void main(String[] args) throws Exception {
        ...
        /**
         ...
         那么代码中的第一段求和就是前面的不变，后面的0-5相加，再前面的变，在0-5相加，空间上0-5是连续的，一次可能六个全部读取，可能读取次数是1024*1024
